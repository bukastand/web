...
export default function HomeContent({
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Link href={`/projects/${project.slug}/about`}
                  className="inline-flex items-center justify-center gap-2 px-8 py-4 border border-white/20 text-white font-semibold rounded-xl text-lg hover:bg-white/10 transition-all duration-300">
                  Pelajari Lebih
                </Link>
              </div>
            </div>
            <div className="hidden lg:flex items-center justify-center">
              <div className={`relative w-80 h-96 rounded-3xl bg-gradient-to-br ${project.gradient} p-1`}>
                <div className="w-full h-full rounded-3xl bg-[#0f172a]/90 flex items-center justify-center overflow-hidden relative">
                  <div className={`absolute inset-0 bg-gradient-to-br ${project.gradient} opacity-20`} />
                  <div className="text-center relative z-10 p-8">
                    <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-white/10 flex items-center justify-center border border-white/10">
                    ...
                    <div className="mt-6 flex flex-wrap justify-center gap-2">
                      {project.techStack.slice(0, 3).map((t) => (
                        <span key={t} className="px-2 py-1 text-[10px] bg-white/10 text-gray-300 rounded-md">{t}</span>
                      ))}
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 reveal-section opacity-0 translate-y-8 transition-all duration-700">
            {stats.map((s, i) => (
              <div key={i} className="text-center">
                <div className="text-3xl md:text-4xl font-extrabold text-[#22c55e] mb-1">{s.val}</div>
      ...
      <section className="relative py-24 sm:py-32 bg-[#0f172a]">
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center reveal-section opacity-0 translate-y-8 transition-all duration-700">
            <div>
              <span className="text-[#22c55e] text-sm font-semibold tracking-widest uppercase mb-4 block">Tentang</span>
              <h2 className="text-3xl sm:text-4xl font-bold text-white mb-6">
                Tentang <span className="gradient-text">{project.title}</span>
              </h2>
              <p className="text-gray-300 leading-relaxed mb-6">{project.aboutSection.story}</p>
              <Link href={`/projects/${project.slug}/about`}
                className="inline-flex items-center gap-2 text-[#22c55e] font-semibold hover:gap-4 transition-all">
                Selengkapnya
      ...
      {/* ===== 4. FEATURES ===== */}
      <section className="relative py-24 sm:py-32 bg-[#0a0f1e]">
        ...
        <div className="container mx-auto px-6">
          <div className="text-center mb-16 reveal-section opacity-0 translate-y-8 transition-all duration-700">
            <span className="text-[#22c55e] text-sm font-semibold tracking-widest uppercase mb-4 block">Fitur Premium</span>
            <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">
              Keunggulan <span className="gradient-text">Website</span> Ini
            </h2>
            <p className="text-gray-400 max-w-2xl mx-auto">Fitur-fitur unggulan yang membuat website ini standout</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {project.features.map((feat, i) => (
              <div key={i}
                className="reveal-section group relative p-6 sm:p-8 rounded-2xl bg-white/5 border border-white/10 hover:border-[#22c55e]/40 transition-all duration-500 hover:-translate-y-1 opacity-0 translate-y-8"
                style={{ transitionDelay: `${i * 80}ms`, transitionDuration: "700ms" }}>
                <div className="w-12 h-12 rounded-xl bg-[#22c55e]/10 text-[#22c55e] flex items-center justify-center mb-5 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <h4 className="text-lg font-bold text-white mb-3">{feat}</h4>
                <p className="text-sm text-gray-400 leading-relaxed line-clamp-3">{project.longDescription}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-24 bg-[#0f172a]">
        <div className="container mx-auto px-6 text-center reveal-section opacity-0 translate-y-8 transition-all duration-700">
          <span className="text-[#22c55e] text-sm font-semibold tracking-widest uppercase mb-4 block">Teknologi</span>
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">
            Tech <span className="gradient-text">Stack</span>
          </h2>
          <p className="text-gray-400 mb-12">Teknologi modern yang digunakan</p>
          <div className="flex flex-wrap justify-center gap-4">
            {project.techStack.map((tech) => (
              <div key={tech}
                className="px-6 py-4 rounded-2xl bg-white/5 border border-white/10 hover:border-[#22c55e]/40 transition-all duration-300 hover:-translate-y-1">
      ...
      <section className="relative py-24 sm:py-32 bg-[#0a0f1e]">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16 reveal-section opacity-0 translate-y-8 transition-all duration-700">
            <span className="text-[#22c55e] text-sm font-semibold tracking-widest uppercase mb-4 block">Alur Kerja</span>
            <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">
      ...
      {/* ===== 7. TESTIMONIALS ===== */}
      <section className="relative py-24 sm:py-32 bg-[#0f172a]">
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-[#22c55e]/5 rounded-full blur-[120px] pointer-events-none" />
        <div className="container mx-auto px-6">
          <div className="text-center mb-16 reveal-section opacity-0 translate-y-8 transition-all duration-700">
            <span className="text-[#22c55e] text-sm font-semibold tracking-widest uppercase mb-4 block">Testimonial</span>
            <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <div key={i}
                className="reveal-section p-6 sm:p-8 rounded-2xl bg-white/5 border border-white/10 hover:border-[#22c55e]/30 transition-all duration-500 opacity-0 translate-y-8"
        ...
        <div className="container mx-auto px-6 text-center relative z-10">
          <div className="reveal-section max-w-3xl mx-auto opacity-0 translate-y-8 transition-all duration-700">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-6">
              Ingin Membuat Website<br /><span className="gradient-text">Seperti Ini?</span>
            </h2>
            <p className="text-lg text-gray-300 mb-10 max-w-2xl mx-auto">
              Konsultasi gratis dengan tim kami. Diskusikan kebutuhan website Anda dan dapatkan solusi terbaik.
            </p>
            <a href={CTA_LINK} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-3 px-10 py-5 bg-[#22c55e] text-white font-bold rounded-2xl text-xl hover:bg-[#16a34a] transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-[#22c55e]/30 animate-pulse-glow">
      ...
      <section className="py-24 bg-[#0f172a]">
        <div className="container mx-auto px-6">
          <div className="text-center mb-12 reveal-section opacity-0 translate-y-8 transition-all duration-700">
            ...
            <p className="text-gray-400">Lihat project lain yang telah kami kerjakan</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {otherProjects.map((p) => (
              <Link key={p.slug} href={`/projects/${p.slug}`}
                className="group rounded-2xl overflow-hidden border border-white/10 hover:border-[#22c55e]/40 transition-all duration-500 hover:-translate-y-2">
                <div className={`h-40 bg-gradient-to-br ${p.gradient} flex items-center justify-center relative`}>
                  <span className="absolute top-3 left-3 px-2 py-0.5 bg-black/40 text-white text-xs rounded-full">{p.category}</span>
