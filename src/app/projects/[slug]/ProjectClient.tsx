    ...
    <div className="min-h-screen bg-[#0f172a]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
            <div className="lg:col-span-3">
              <div className="opacity-0 translate-y-8 transition-all duration-700">
                <h2 className="text-3xl sm:text-4xl font-bold text-white mb-6">Tentang Project Ini</h2>
                <p className="text-lg text-gray-300 leading-relaxed mb-8">{project.longDescription}</p>
              </div>
              <div className="opacity-0 translate-y-8 transition-all duration-700 delay-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {project.features.map((feat, i) => (
                    <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-[#22c55e]/30 transition-all duration-300">
                ...
                <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.techStack.map((tech) => (
                      <span key={tech} className="px-3 py-1.5 text-xs font-medium bg-[#22c55e]/10 text-[#22c55e] rounded-full border border-[#22c55e]/20">
                        {tech}
                      </span>
                    ))}
                  </div>
                </div>
                <a href={heroData.cta_link} target="_blank" rel="noopener noreferrer"
      ...
      <section className="py-24 bg-[#0f172a]">
        <div className="container mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-400 max-w-xl mx-auto">Lihat project lain yang telah kami kerjakan</p>
          </div>
            ...
            {otherProjects.map((p) => (
              <Link key={p.slug} href={`/projects/${p.slug}`}
                ...
                <div className={`h-36 bg-gradient-to-br ${p.gradient} flex items-center justify-center relative`}>
                  <span className="absolute top-2 left-2 px-2 py-0.5 bg-black/40 text-white text-xs rounded-full">{p.category}</span>
          ...
          <div className="text-center mt-10">
            <Link href="/#portfolio" className="inline-flex items-center gap-2 text-[#22c55e] hover:text-[#4ade80] transition-colors font-medium">
      ...
      <footer className="py-8 bg-[#060a14] border-t border-white/5">
        <div className="container mx-auto px-6 text-center">
          <p className="text-sm text-gray-600">© {new Date().getFullYear()} PAGODA STUDIO — Jasa Website Profesional.</p>
        </div>
      </footer>
    </div>
  );
