  ...
  return (
    <>
      ...
      <section className="relative py-24 bg-white">
        <div className="container mx-auto px-6">
          <div className="max-w-4xl mx-auto reveal-section opacity-0 translate-y-8 transition-all duration-700">
            <span className="text-accent text-sm font-semibold tracking-widest uppercase mb-4 block text-center">Cerita Kami</span>
            <h2 className="text-3xl sm:text-4xl font-bold text-ink mb-8 text-center">
              Perjalanan <span className="text-accent">{project.title}</span>
            </h2>
            <div className="relative p-8 sm:p-12 rounded-3xl bg-surface border border-line">
              <div className={`absolute top-0 left-0 w-1 h-full bg-gradient-to-b ${project.gradient} rounded-l-3xl`} />
              <p className="text-muted leading-relaxed text-lg">{aboutSection.story}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-24 bg-surface">
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <div className="reveal-section p-8 sm:p-10 rounded-3xl bg-surface border border-line opacity-0 translate-y-8 transition-all duration-700">
              <div className="w-14 h-14 rounded-xl bg-accent/20 flex items-center justify-center mb-6">
                <svg className="w-7 h-7 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <h3 className="text-2xl font-bold text-ink mb-4">Misi</h3>
              <p className="text-muted leading-relaxed">{aboutSection.mission}</p>
            </div>
            <div className="reveal-section p-8 sm:p-10 rounded-3xl bg-surface border border-line opacity-0 translate-y-8 transition-all duration-700"
              ...
              <h3 className="text-2xl font-bold text-ink mb-4">Visi</h3>
              <p className="text-muted leading-relaxed">{aboutSection.vision}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-24 bg-white">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16 reveal-section opacity-0 translate-y-8 transition-all duration-700">
            <span className="text-accent text-sm font-semibold tracking-widest uppercase mb-4 block">Nilai-Nilai</span>
            <h2 className="text-3xl sm:text-4xl font-bold text-ink mb-4">
              Core <span className="text-accent">Values</span>
            </h2>
            <p className="text-muted max-w-2xl mx-auto">Prinsip yang menjadi fondasi setiap keputusan dan tindakan kami</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {aboutSection.values.map((value, i) => (
              <div key={i}
                className="reveal-section group p-6 rounded-2xl bg-surface border border-line hover:border-accent/40 transition-all duration-500 hover:-translate-y-1 opacity-0 translate-y-8"
                ...
                <h4 className="text-lg font-bold text-ink">{value}</h4>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-24 bg-surface">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16 reveal-section opacity-0 translate-y-8 transition-all duration-700">
            <span className="text-accent text-sm font-semibold tracking-widest uppercase mb-4 block">Tim Kami</span>
            <h2 className="text-3xl sm:text-4xl font-bold text-ink mb-4">
              <span className="text-accent">Tim</span> Profesional
            </h2>
            <p className="text-muted max-w-2xl mx-auto">Kenalan dengan orang-orang hebat di balik {project.title}</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {aboutSection.teamMembers.map((member, i) => (
              <div key={i}
                className="reveal-section group text-center p-6 rounded-2xl bg-surface border border-line hover:border-accent/30 transition-all duration-500 hover:-translate-y-2 opacity-0 translate-y-8"
                style={{ transitionDelay: `${i * 150}ms`, transitionDuration: "700ms" }}>
                <div className={`w-20 h-20 mx-auto mb-5 rounded-2xl bg-gradient-to-br ${project.gradient} flex items-center justify-center text-xl font-bold text-ink group-hover:scale-110 transition-transform duration-500`}>
                  {member.avatar}
                </div>
                <h4 className="text-lg font-bold text-ink mb-1">{member.name}</h4>
